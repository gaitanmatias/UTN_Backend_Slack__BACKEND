...
class AuthController {
  static async register(request, response) {
    try {
      ...
      console.log("Usuario registrado con exito ", `usuario: ${username}; email: ${email}; password: ${password.slice(0,4)+'****'}` );

      if (!username) {
        throw new ServerError(400, "Debes enviar un nombre de usuario valido");
      } else if (
        !email ||
        !String(email)
          .toLowerCase()
  ...
  static async login(request, response) {
    try {
      ...
      console.log("Usuario registrado con exito ", `email: ${email}; password: ${password.slice(0,4)+'****'}` );
